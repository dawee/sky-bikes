...
const render = templates => {
  const headerComponent = renderHeader(templates);

  return (props, node) => {
    const { header, members, stations } = props;
    const headerNode = node.querySelector('.header');
    const stationsListBodyNode = node.querySelector(
      '.station-slots-list tbody'
    );
    ...
    if (headerNode) {
      headerComponent(header, headerNode);
    } else {
      node.insertBefore(
        headerComponent(header),
        node.querySelector('.stations-list-title')
      );
    }
